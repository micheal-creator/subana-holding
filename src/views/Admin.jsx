...
function CollectionEditor({ section, cms, setCms }) {
  ...
  const fields = FIELD_CONFIG[section] || []
  async function saveItem(item) { const next = editing?.index == null ? [...items, { ...item, id: item.id || `${section}-${Date.now()}` }] : items.map((x, i) => i === editing.index ? { ...x, ...item } : x); try { await setCms({ ...cms, [section]: next }); setEditing(null) } catch { alert('Could not save to Supabase. Check that your account has is_admin=true in public.profiles.') } }
  async function remove(index) { if (confirm('Remove this item from the website?')) { try { await setCms({ ...cms, [section]: items.filter((_, i) => i !== index) }) } catch { alert('Could not save this change to Supabase.') } } }
...
function SettingsEditor({ cms, setCms }) {
  ...
  async function save() { try { await setCms({ ...cms, settings: draft }); alert('Site settings saved to Supabase.') } catch { alert('Could not save to Supabase. Check that your account has is_admin=true in public.profiles.') } }
...
export default function Admin() {
  const [authed, setAuthed] = useState(() => !hasSupabase && localStorage.getItem('subana:admin') === '1')
  const [checking, setChecking] = useState(hasSupabase)
  const [isAdmin, setIsAdmin] = useState(!hasSupabase)
